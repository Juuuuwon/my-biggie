...
func KafkaHeavyHandler(c *gin.Context) {
	...
	stressFunc := func() {
		...
		writer.Close()
		logger.Info("Kafka heavy produce (single producer) completed", zap.Int("duration_sec", maintainSec))
	}
...
func KafkaMultiHeavyHandler(c *gin.Context) {
	...
	stressFunc := func() {
		...
		wg.Wait()
		logger.Info("Kafka multi heavy produce completed", zap.Int("producers", connectionCounts))
	}
...
func KafkaConnectionHandler(c *gin.Context) {
	...
	Loop:
		...
		mu.Unlock()
		logger.Info("Kafka connection stress completed", zap.Int("producers", currentCount))
	}
